// src/routes/bhprd.routes.js
const express = require('express');
const router = express.Router();
const multer = require('multer');
const path = require('path');
const { auth, checkRole } = require('../middlewares/auth');
const bhprdController = require('../controllers/bhprd.controller');

// Configure multer for file upload
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, 'storage/uploads/temp');
  },
  filename: function (req, file, cb) {
    const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
    cb(null, 'bhprd-' + uniqueSuffix + path.extname(file.originalname));
  }
});

const upload = multer({
  storage: storage,
  limits: {
    fileSize: 10 * 1024 * 1024 // 10MB max file size
  },
  fileFilter: function (req, file, cb) {
    const ext = path.extname(file.originalname).toLowerCase();
    if (ext !== '.json') {
      return cb(new Error('Hanya file JSON yang diperbolehkan'));
    }
    cb(null, true);
  }
});

/**
 * @route   POST /api/bhprd/upload
 * @desc    Upload and replace bhprd2025.json file
 * @access  Private - superadmin, sarana_prasarana only
 */
router.post(
  '/upload',
  auth,
  checkRole('superadmin', 'sarana_prasarana'),
  upload.single('file'),
  bhprdController.uploadBhprdData
);

/**
 * @route   GET /api/bhprd/data
 * @desc    Get bhprd2025.json data
 * @access  Private - authenticated users
 */
router.get(
  '/data',
  auth,
  bhprdController.getBhprdData
);

/**
 * @route   GET /api/bhprd/info
 * @desc    Get current BHPRD data information
 * @access  Private - authenticated users
 */
router.get(
  '/info',
  auth,
  bhprdController.getBhprdInfo
);

/**
 * @route   GET /api/bhprd/backups
 * @desc    Get list of backup files
 * @access  Private - superadmin, sarana_prasarana only
 */
router.get(
  '/backups',
  auth,
  checkRole('superadmin', 'sarana_prasarana'),
  bhprdController.getBackupList
);

module.exports = router;
