// src/controllers/dd-earmarked-t2.controller.js
const fs = require('fs').promises;
const path = require('path');
const ActivityLogger = require('../utils/activityLogger');

/**
 * Upload and replace dd-earmarked-tahap2.json file
 */
const uploadDdEarmarkedT2Data = async (req, res) => {
  try {
    ...
    const targetFile = path.join(publicDir, 'dd-earmarked-tahap2.json');
    ...
    // Move uploaded file to replace dd-earmarked-tahap2.json
    ...
    await ActivityLogger.log({
      ...
      module: 'dd',
      action: 'upload',
      entityType: 'dd_earmarked_tahap2',
      description: `Upload data DD Earmarked Tahap 2: ${totalRows} baris, ${validRows} valid, ${uniqueDesa} desa`,
 ...
 */
const getDdEarmarkedT2Data = async (req, res) => {
  try {
    const publicDir = path.join(__dirname, '../../public');
    const targetFile = path.join(publicDir, 'dd-earmarked-tahap2.json');
 ...
 */
const getDdEarmarkedT2Info = async (req, res) => {
  try {
    const publicDir = path.join(__dirname, '../../public');
    const targetFile = path.join(publicDir, 'dd-earmarked-tahap2.json');
    ...
    res.json({
      success: true,
      data: {
        fileName: 'dd-earmarked-tahap2.json',
        fileSize: stats.size,
 ...
 */
const getDdEarmarkedT2BackupList = async (req, res) => {
  try {
    const publicDir = path.join(__dirname, '../../public');
    const backupDir = path.join(publicDir, 'backups');
...
module.exports = {
  uploadDdEarmarkedT2Data,
  getDdEarmarkedT2Data,
  getDdEarmarkedT2Info,
  getDdEarmarkedT2BackupList
};
