// src/controllers/dd-earmarked-t1.controller.js
const fs = require('fs').promises;
const path = require('path');
const ActivityLogger = require('../utils/activityLogger');

/**
 * Upload and replace dd-earmarked-tahap1.json file
 */
const uploadDdEarmarkedT1Data = async (req, res) => {
  try {
    ...
    const targetFile = path.join(publicDir, 'dd-earmarked-tahap1.json');
    ...
    // Move uploaded file to replace dd-earmarked-tahap1.json
    ...
    await ActivityLogger.log({
      ...
      module: 'dd',
      action: 'upload',
      entityType: 'dd_earmarked_tahap1',
      description: `Upload data DD Earmarked Tahap 1: ${totalRows} baris, ${validRows} valid, ${uniqueDesa} desa`,
 ...
 */
const getDdEarmarkedT1Data = async (req, res) => {
  try {
    const publicDir = path.join(__dirname, '../../public');
    const targetFile = path.join(publicDir, 'dd-earmarked-tahap1.json');
 ...
 */
const getDdEarmarkedT1Info = async (req, res) => {
  try {
    const publicDir = path.join(__dirname, '../../public');
    const targetFile = path.join(publicDir, 'dd-earmarked-tahap1.json');
    ...
    res.json({
      success: true,
      data: {
        fileName: 'dd-earmarked-tahap1.json',
        fileSize: stats.size,
 ...
 */
const getDdEarmarkedT1BackupList = async (req, res) => {
  try {
    const publicDir = path.join(__dirname, '../../public');
    const backupDir = path.join(publicDir, 'backups');
...
module.exports = {
  uploadDdEarmarkedT1Data,
  getDdEarmarkedT1Data,
  getDdEarmarkedT1Info,
  getDdEarmarkedT1BackupList
};
