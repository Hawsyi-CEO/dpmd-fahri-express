// src/controllers/dd-nonearmarked-t1.controller.js
const fs = require('fs').promises;
const path = require('path');
const ActivityLogger = require('../utils/activityLogger');

/**
 * Upload and replace dd-nonearmarked-tahap1.json file
 */
const uploadDdNonEarmarkedT1Data = async (req, res) => {
  try {
    ...
    const targetFile = path.join(publicDir, 'dd-nonearmarked-tahap1.json');
    ...
    try {
      ...
      const backupFile = path.join(backupDir, `dd2025_backup_${timestamp}.json`);
    ...
    // Move uploaded file to replace dd-nonearmarked-tahap1.json
    ...
    await ActivityLogger.log({
      ...
      module: 'dd',
      action: 'upload',
      entityType: 'dd_nonearmarked_tahap1',
      description: `Upload data DD Non-Earmarked Tahap 1: ${totalRows} baris, ${validRows} valid, ${uniqueDesa} desa`,
    ...
    res.json({
      success: true,
      message: 'Data DD berhasil diupdate',
      data: {
        totalRows,
        validRows,
        uniqueDesa,
    ...
    console.error('Error uploading DD data:', error);
...
/**
 * Get DD data (JSON content)
 */
const getDdNonEarmarkedT1Data = async (req, res) => {
  try {
    const publicDir = path.join(__dirname, '../../public');
    const targetFile = path.join(publicDir, 'dd-nonearmarked-tahap1.json');
    ...
    try {
      ...
      return res.status(404).json({
        success: false,
        message: 'File data DD tidak ditemukan'
      });
    }
    ...
    console.error('Error getting DD data:', error);
    res.status(500).json({
      success: false,
      message: 'Gagal membaca data DD',
      error: error.message
    });
  }
};
...
/**
 * Get current DD data info
 */
const getDdNonEarmarkedT1Info = async (req, res) => {
  try {
    const publicDir = path.join(__dirname, '../../public');
    const targetFile = path.join(publicDir, 'dd-nonearmarked-tahap1.json');
    ...
    res.json({
      success: true,
      data: {
        fileName: 'dd-nonearmarked-tahap1.json',
        fileSize: stats.size,
    ...
    console.error('Error getting DD info:', error);
    res.status(500).json({
      success: false,
 ...
 */
const getDdNonEarmarkedT1BackupList = async (req, res) => {
  try {
    const publicDir = path.join(__dirname, '../../public');
    const backupDir = path.join(publicDir, 'backups');
    ...
    const backupFiles = files.filter(file => file.startsWith('dd2025_backup_'));
...
module.exports = {
  uploadDdNonEarmarkedT1Data,
  getDdNonEarmarkedT1Data,
  getDdNonEarmarkedT1Info,
  getDdNonEarmarkedT1BackupList
};
