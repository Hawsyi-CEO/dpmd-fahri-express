 ...
 */
const uploadAddData = async (req, res) => {
  try {
    ...
    const targetFile = path.join(publicDir, 'add2025.json');
    ...
    try {
      ...
      const backupFile = path.join(backupDir, `add2025_backup_${timestamp}.json`);
    ...
    // Move uploaded file to replace add2025.json
    ...
    res.json({
      success: true,
      message: 'Data ADD berhasil diupdate',
      data: {
        totalRows,
        validRows,
        uniqueDesa,
    ...
    console.error('Error uploading ADD data:', error);
...
/**
 * Get ADD data (JSON content)
 */
const getAddData = async (req, res) => {
  try {
    const publicDir = path.join(__dirname, '../../public');
    const targetFile = path.join(publicDir, 'add2025.json');
    ...
    try {
      ...
      return res.status(404).json({
        success: false,
        message: 'File data ADD tidak ditemukan'
      });
    }
    ...
    console.error('Error getting ADD data:', error);
    res.status(500).json({
      success: false,
      message: 'Gagal membaca data ADD',
      error: error.message
    });
  }
};
...
const getAddInfo = async (req, res) => {
  try {
    ...
    console.error('Error getting ADD info:', error);
    res.status(500).json({
      success: false,
...
const getBackupList = async (req, res) => {
  try {
    ...
    const backupFiles = files.filter(file => file.startsWith('add2025_backup_'));
