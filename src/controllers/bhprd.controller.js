// src/controllers/bhprd.controller.js
const fs = require('fs').promises;
const path = require('path');

/**
 * Upload and replace bhprd2025.json file
 */
const uploadBhprdData = async (req, res) => {
  try {
    ...
    const targetFile = path.join(publicDir, 'bhprd2025.json');
    ...
    try {
      ...
      const backupFile = path.join(backupDir, `bhprd2025_backup_${timestamp}.json`);
    ...
    // Move uploaded file to replace bhprd2025.json
    ...
    res.json({
      success: true,
      message: 'Data BHPRD berhasil diupdate',
      data: {
        totalRows,
        validRows,
        uniqueDesa,
    ...
    console.error('Error uploading BHPRD data:', error);
...
/**
 * Get BHPRD data (JSON content)
 */
const getBhprdData = async (req, res) => {
  try {
    const publicDir = path.join(__dirname, '../../public');
    const targetFile = path.join(publicDir, 'bhprd2025.json');
    ...
    try {
      ...
      return res.status(404).json({
        success: false,
        message: 'File data BHPRD tidak ditemukan'
      });
    }
    ...
    console.error('Error getting BHPRD data:', error);
    res.status(500).json({
      success: false,
      message: 'Gagal membaca data BHPRD',
      error: error.message
    });
  }
};
...
/**
 * Get current BHPRD data info
 */
const getBhprdInfo = async (req, res) => {
  try {
    const publicDir = path.join(__dirname, '../../public');
    const targetFile = path.join(publicDir, 'bhprd2025.json');
    ...
    console.error('Error getting BHPRD info:', error);
    res.status(500).json({
      success: false,
...
const getBackupList = async (req, res) => {
  try {
    ...
    const backupFiles = files.filter(file => file.startsWith('bhprd2025_backup_'));
